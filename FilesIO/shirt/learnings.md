...
### 1. Class Methods vs. Constructor Instantiation (`Image.open`)
...
### 2. Class Methods vs. Instance Methods (`open`, `paste`, `save`)

- **`Image.open` (Class/Factory Method):** Called directly on the class (`Image.open()`) without needing a pre-existing object. Its primary job is to create and return a new `Image` object.
- **`Image.paste` & `Image.save` (Instance Methods):** These require an existing `Image` object to call them (e.g., `cropped_image.paste(...)` or `cropped_image.save(...)`).
- **In-Place Modification & Return Values:**
  - Methods like `.paste()` operate **in-place**, meaning they directly modify the pixel data of the object calling the method and **return `None`** rather than a new object.
  - In contrast, independent functions like `ImageOps.fit()` do not modify the original object; instead, they process it and return a brand-new `Image` object.
